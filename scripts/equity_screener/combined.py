...
def combined_top25_opportunities(df: pd.DataFrame, limit: int = 25, sector_cap: int = 3) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        contrib_rows.append({
            ...
            "quality_penalty": row.get("quality_penalty"),
            "thin_volume_penalty": row.get("thin_volume_penalty"),
            "broken_trend_penalty": row.get("broken_trend_penalty"),
            "crash_penalty": row.get("crash_penalty"),
            "ev_score": ev,
