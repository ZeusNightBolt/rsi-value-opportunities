...
def render_dashboard(df: pd.DataFrame, analyses: list[dict], price_filter: float) -> None:
    ...
    for display_rank, (_, r) in enumerate(combined_top25.iterrows(), 1):
        ...
        combined_rows.append(
            ...
            f"<td><strong>{ticker_link(rr['ticker'])}</strong><small>{html.escape(str(rr.get('company', ''))[:42])}</small></td>"
        ...
        combined_cards.append(
            ...
            f"<div><span>{html.escape(status)}</span><strong>{ticker_link(str(rr.get('ticker', '')))}</strong><em>{html.escape(str(rr.get('production_factor_basket', '')))}</em></div>"
            f"<p><b class='combined-rank-score'>Combined {fmt_num(rr.get('combined_rank_score'))}</b> · Opp {fmt_num(rr.get('opportunity_score'))} · EV {fmt_num(rr.get('ev_score'))}</p>"
            f"<small>Theme {html.escape(str(rr.get('primary_keyword_factor', '')))} · factor {fmt_num(rr.get('production_factor_score'))} · theme score {fmt_num(rr.get('primary_keyword_factor_score'))}</small>"
            "</article>"
        )
    div_rows = []
    div_cards = []
...
<div class="tab-content" id="c-top25">{stage_intro('Composite','Combined Top 25 opportunities','Rank-sorted blend of opportunity score, EV master score, production factor basket, keyword theme score, and confirmation/divergence status.', len(combined_rows))}<p class="note"><span class="pill">Top 25 combined ranking</span> Confirmation + high composite score moves a name up; Broken Momentum / Avoid stays visible as divergence but receives a penalty. Sector cap is four names.</p><div class="factor-alignment-grid">{''.join(combined_cards)}</div><div class="data-panel"><div class="panel-title"><h3>Top 25 combined ranking</h3><span>sectors + themes + factor baskets</span></div><div class="table-wrap"><table><thead><tr><th>#</th><th>Ticker</th><th>Confirmation</th><th>Combined</th><th>Opp</th><th>EV</th><th>Factor</th><th>Theme</th><th>Theme / basket</th><th>Takeaway</th></tr></thead><tbody>{''.join(combined_rows)}</tbody></table></div></div></div>
